...
def summarize_candidate(
    ...
    gains = {
        policy: accuracy - float(score) for policy, score in baseline_means.items()
    }
    minimum_policy_gain = min(gains.values())
    clearly_outperformed = [policy for policy, gain in gains.items() if gain >= 0.02]
    valid = (
        stable
        and checkpoint
        and budget_pass
...
def main() -> int:
    ...
    for candidate in CANDIDATES:
        ...
        summaries.append(summarize_candidate(candidate, rows, records, baseline_means))
